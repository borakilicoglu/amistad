...
export function Dashboard({ data, summary }: Props) {
  ...
  return (
    ...
    >
      <main className="grid gap-7 px-16 max-md:px-8">
        <section className="grid grid-cols-2 items-stretch gap-6 max-xl:grid-cols-1">
          <div className="grid self-start gap-0">
            {jobs.length === 0 ? (
              <div className={`${panelClass} p-6 text-muted`}>
                No jobs match the current filters.
              </div>
            ) : (
              jobs.map((job) => (
                <div
                  className={`job-item ${job.id === selectedJob?.id ? "job-item-active" : ""}`}
                  key={job.id}
                >
                  <JobRow
                    job={job}
          ...
          <div className="sticky top-38 self-start max-xl:static">
            <section className="rounded-3xl bg-detail-panel">
              {selectedJob ? (
                <JobDetail job={selectedJob} />
              ) : (
                <div className="p-6 text-muted">Select a job.</div>
              )}
            </section>
          </div>
        </section>
